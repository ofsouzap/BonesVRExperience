...
{
    public abstract class LiveHeight : MonoBehaviour
    {
        private static readonly Color c_LiveHeightGizmoColor = new(1f, .3f, .3f, .3f);
        private const float c_GizmoWidth = .2f;

        protected abstract float LiveMinHeight { get; }
        ...
        protected virtual bool CheckShouldDrawGizmo() => true;

        protected virtual void OnDrawGizmosSelected()
        {
            if (CheckShouldDrawGizmo())
            {
